...
def project_image_to_cam(uv_depth, K):
    ...
    n = uv_depth.shape[0]
    eps = 1e-8
    fx_safe = f_u if abs(f_u) > eps else eps
    fy_safe = f_v if abs(f_v) > eps else eps

    # Filter out zero, negative, NaN, and Inf depth values
    valid_depth = np.isfinite(uv_depth[:, 2]) & (uv_depth[:, 2] > eps)
    uv_depth = uv_depth[valid_depth]

    if uv_depth.shape[0] == 0:
        return np.zeros((0, 3))

    x = ((uv_depth[:,0]-c_u)*uv_depth[:,2])/fx_safe
    y = ((uv_depth[:,1]-c_v)*uv_depth[:,2])/fy_safe
    pts_3d_rect = np.zeros((uv_depth.shape[0], 3))
    pts_3d_rect[:,0] = x
    pts_3d_rect[:,1] = y
