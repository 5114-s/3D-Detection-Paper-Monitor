...
# 改进2: MoGe+Depth Pro融合
from .depth_predictor.moge_depthpro_fusion import MoGeDepthProFusion, create_moge_depthpro_fusion
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        # 改进2: 支持MoGe+Depth Pro融合或单一UniDepth
        self.use_moge_depthpro = getattr(cfg, 'use_moge_depthpro', False)
        
        if self.use_moge_depthpro:
            # 导入融合模块
            from .depth_predictor.moge_depthpro_fusion import (
                MoGeLoader, DepthProLoader, create_moge_depthpro_fusion
            )
            
            # 获取模型路径
            moge_path = getattr(cfg, 'moge_path', None)
            depthpro_path = getattr(cfg, 'depthpro_path', None)
            fusion_method = getattr(cfg, 'fusion_method', 'ransac_align')
            
            # 加载MoGe模型
            moge_loader = MoGeLoader(device=self.device)
            moge_model = None
            try:
                moge_model = moge_loader.load_model(moge_path)
            except Exception as e:
                print(f"警告: MoGe模型加载失败: {e}")
            
            # 加载Depth Pro模型
            depthpro_loader = DepthProLoader(device=self.device)
            depthpro_model = None
            try:
                depthpro_model = depthpro_loader.load_model(depthpro_path)[0]
            except Exception as e:
                print(f"警告: Depth Pro模型加载失败: {e}")
            
            if moge_model is not None and depthpro_model is not None:
                # 创建融合模型
                self.depth_head = create_moge_depthpro_fusion(
                    moge_model=moge_loader,
                    depthpro_model=depthpro_loader,
                    fusion_method=fusion_method,
                )
                print(f">> 使用MoGe+Depth Pro融合深度模型 (方法: {fusion_method})")
            else:
                # 回退到UniDepth
                print(f">> MoGe和Depth Pro未完全加载,使用UniDepth作为后备")
                from .depth_predictor.unidepth import DepthHead
                self.depth_head = DepthHead(cfg)
                self.use_moge_depthpro = False
                print(">> 使用单一UniDepth深度模型")
        else:
            # 使用单一UniDepth
            from .depth_predictor.unidepth import DepthHead
            self.depth_head = DepthHead(cfg)
